extends Node
## Stage 1 (v2) CLI Runner
## Port of tools/tileset_builder.gd to a runnable Node for headless execution.
...
func _ready() -> void:
    print("=== Stage 1 v2: auto TileSet builder (CLI) ===")
    ...
    for name in coords:
        ...
        if td == null: continue
        if not FILE_TO_TERRAIN.has(name): continue
    ...
    for y in range(0, img.get_height(), 2):
        for x in range(0, img.get_width(), 2):
            ...
            if px.a > 0.5:
                sum += px
                n += 1
    if n == 0: return TERRAIN_COLORS[0]
