extends SceneTree
## Генератор WAV-плейсхолдеров для звуков.
## Запуск: godot --headless -s tools/sound_synth.gd
...
func _init() -> void:
    ...
    _generate("attack_hit", 0.15, 150.0, 0.5) # Noise burst
    _generate("unit_death", 0.4, 400.0, -15.0) # Descending tone
    _generate("pickup_item", 0.2, 1200.0, 5.0) # Ascending coin
    _generate("bgm_loop", 4.0, 220.0, 0.0) # Simple ambient
...
func _generate(name: String, duration: float, base_freq: float, freq_slope: float) -> void:
    ...
    data.resize(num_samples * 2) # 16-bit PCM
