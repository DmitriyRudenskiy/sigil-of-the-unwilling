...
import { normalizeParameters, convertCamelToSnakeCase, validatePath, createErrorResponse, isGodot44OrLater, generateGodotProjectFeatures, generateCsprojContent, generateCsharpScriptSource, toDotnetIdentifier, isValidCsharpIdentifier, parseGodotScriptDiagnostics, collectGdPaths, } from './utils.js';
...
class GodotServer {
    ...
    async sendGameCommand(command, params = {}, timeoutMs = 10000) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.gameConnection.pendingRequests.set(id, (response) => {
    ...
    async runGdScriptCheck(projectPath, scriptFull) {
        ...
        try {
            ...
            const { stdout, stderr } = await execFileAsync(this.godotPath, ['--headless', '--path', projectPath, '--script', this.validateScriptPath, scriptRes], { timeout: 30000, maxBuffer: 16 * 1024 * 1024 });
            ...
            if (!output)
                return { completed: false, errors: [], error: error?.message || 'Unknown error' };
        }
