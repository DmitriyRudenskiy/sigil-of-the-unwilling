...
func stop() -> void:
	...
	for _oid in _orphan_ids_at_stop:
		var _o := instance_from_id(_oid)
		var _cls := (_o.get_class() if is_instance_valid(_o) else "null")
		var _scr := "<no script>"
		var _par := "<no parent>"
		if _o is Node:
			_scr = str((_o.get_script() as Script).get_path() if is_instance_valid((_o.get_script() as Script)) else _scr)
			var _p: Variant = _o.get_parent()
			_par = ("<no parent>" if _p == null else (((_p as Node).get_script() as Script).get_path() if (_p as Node).get_script() else (_p as Node).get_class()))
		print("ORPHAN_PROBE class=", _cls, " name=", (_o.name if _o is Node else ""), " parent=", _par)
...
func collect() -> void:
	...
	for orphan_id in _orphan_ids_at_stop:
		...
		print("ORPHAN_PROBE class=", (orphan_node.get_class() if is_instance_valid(orphan_node) else "null"))
