...
func _ready() -> void:
	# Канонический контракт поиска для систем-потребителей (CrisisEventSystem):
	# GameManager может находиться как в дереве Main (main.tscn), так и быть
	# смонтирован напрямую на /root в тестах — группа работает в обоих случаях.
