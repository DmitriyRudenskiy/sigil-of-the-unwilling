...
static func get_all_neighbors(cell: Vector2i, shift_right: bool = true) -> Array[Vector2i]:
	var r: Array[Vector2i] = []
	r.resize(6)
	for i in 6:
		r[i] = get_neighbor(cell, i, shift_right)
	return r

static func offset_to_cube(cell: Vector2i, shift_right: bool = true) -> Vector3i:
	var r := cell.y
	var x: int
	if shift_right:
		x = cell.x - int((r - (r & 1)) / 2.0)
	else:
		x = cell.x - int((r + (r & 1)) / 2.0)
	var z := r
	return Vector3i(x, -x - z, z)

static func cube_to_offset(c: Vector3i, shift_right: bool = true) -> Vector2i:
	var r := c.z
	var x: int
	if shift_right:
		x = c.x + int((r - (r & 1)) / 2.0)
...
static func hex_distance(a: Vector2i, b: Vector2i, shift_right: bool = true) -> int:
	var ac := offset_to_cube(a, shift_right)
	var bc := offset_to_cube(b, shift_right)
	return max(max(absi(ac.x - bc.x), absi(ac.y - bc.y)), absi(ac.z - bc.z))

static func ring(center: Vector2i, r: int, shift_right: bool = true) -> Array[Vector2i]:
	if r <= 0:
		return [center]
	...
	var cube := offset_to_cube(center, shift_right)
	var cur := cube + Vector3i(dirs[4].x, -dirs[4].x - dirs[4].y, dirs[4].y) * r
	for i in 6:
		for _j in r:
			out.append(cube_to_offset(cur, shift_right))
