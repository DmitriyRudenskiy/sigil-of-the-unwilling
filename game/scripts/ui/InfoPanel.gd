...
@onready var _end_turn_btn: Button = $columns/btn_col_right/EndTurnButton
...
func _ready() -> void:
	...
	for col_name in ["columns/btn_col_left", "columns/btn_col_right"]:
		for child: Node in (get_node(col_name) as Control).get_children():
			if child is Button:
