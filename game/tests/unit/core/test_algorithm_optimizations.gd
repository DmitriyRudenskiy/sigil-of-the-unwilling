...
func test_astar_finds_path() -> void:
	...
	var path := HexPathfinding.astar_path(start, goal, blocked, 21, 21)
...
func test_astar_no_path() -> void:
	...
	for nb in HexUtils.get_all_neighbors(goal):
		blocked[nb] = true
	var path := HexPathfinding.astar_path(Vector2i(0, 0), goal, blocked, 21, 21)
...
func test_astar_same_as_dijkstra_length() -> void:
	...
	var start := Vector2i(0, 0)
	var goal := Vector2i(10, 5)
	var dist := HexPathfinding.dijkstra(start, 100.0, cost_fn, 21, 21)
	var goal_idx := HexUtils.pos_to_idx(goal, 21)
	if dist[goal_idx] >= 1e9:
		return
	var dijkstra_p := HexPathfinding.dijkstra_path(start, goal, dist, cost_fn, 21, 21)
	var astar_p := HexPathfinding.astar_path(start, goal, blocked, 21, 21)
...
func test_find_path_dispatch_matches_underlying() -> void:
	...
	var start := Vector2i(0, 0)
	var goal := Vector2i(10, 5)
	var via_dispatch := HexPathfinding.find_path(start, goal, blocked, 21, 21)
	var via_astar := HexPathfinding.astar_path(start, goal, blocked, 21, 21)
	assert_that(via_dispatch).is_equal(via_astar)

	var via_bfs := HexPathfinding.find_path(start, goal, blocked, 21, 21, true, "bfs")
	var direct_bfs := HexPathfinding.bfs_path(start, goal, blocked, 21, 21)
	assert_that(via_bfs).is_equal(direct_bfs)

	var surrounded := Vector2i(10, 10)
	for nb in HexUtils.get_all_neighbors(surrounded):
		blocked[nb] = true
	assert_bool(HexPathfinding.find_path(Vector2i(0, 0), surrounded, blocked, 21, 21).is_empty()).is_true()
