...
func test_cursor_overlay_mode() -> void:
	...
	assert_int(c.mode).is_equal(_BattleView.CursorMode.ATTACK).override_failure_message("cursor overlay mode should be ATTACK after set_cursor_mode")

	c.set_mode(_BattleView.CursorMode.SPELL)
	assert_int(c.mode).is_equal(_BattleView.CursorMode.SPELL).override_failure_message("cursor overlay mode should be SPELL after set_cursor_mode")

	c.set_mode(_BattleView.CursorMode.RANGED)
	assert_int(c.mode).is_equal(_BattleView.CursorMode.RANGED).override_failure_message("cursor overlay mode should be RANGED after set_cursor_mode")

	c.set_mode(_BattleView.CursorMode.DEFAULT)
	assert_int(c.mode).is_equal(_BattleView.CursorMode.DEFAULT).override_failure_message("cursor overlay mode should be DEFAULT after set_cursor_mode")
...
func test_view_cursor_methods() -> void:
	...
	assert_int(view._cursor.mode).is_equal(_BattleView.CursorMode.SPELL).override_failure_message("view.set_cursor_mode should propagate to overlay")
	...
	view.clear_cursor()
	assert_int(view._cursor.mode).is_equal(_BattleView.CursorMode.DEFAULT).override_failure_message("view.clear_cursor should reset mode to DEFAULT")
...
func test_input_setter_propagates() -> void:
	...
	assert_int(input._cursor_mode).is_equal(_BattleView.CursorMode.ATTACK).override_failure_message("input cursor mode should be ATTACK after set_cursor_mode")

	input.set_cursor_mode(_BattleView.CursorMode.SPELL)
	assert_int(input._cursor_mode).is_equal(_BattleView.CursorMode.SPELL).override_failure_message("input cursor mode should be SPELL after set_cursor_mode")

	input.set_cursor_mode(_BattleView.CursorMode.RANGED)
	assert_int(input._cursor_mode).is_equal(_BattleView.CursorMode.RANGED).override_failure_message("input cursor mode should be RANGED after set_cursor_mode")
...
func test_walk_cursor_distinct_from_ranged() -> void:
	...
	assert_int(_BattleView.CursorMode.MOVE).is_not_equal(_BattleView.CursorMode.DEFAULT).override_failure_message("MOVE cursor mode must be distinct from DEFAULT")
