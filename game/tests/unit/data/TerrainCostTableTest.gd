...
func test_cost_forest() -> void:
	assert_that(_TerrainCostTable.get_cost("forest")).is_equal(1.25)

func test_cost_sand() -> void:
	assert_that(_TerrainCostTable.get_cost("sand")).is_equal(1.5)

func test_cost_swamp() -> void:
	assert_that(_TerrainCostTable.get_cost("swamp")).is_equal(1.75)

func test_cost_water() -> void:
	assert_bool(_TerrainCostTable.get_cost("water") >= INF - 1).is_true()

func test_cost_levitation_water() -> void:
	assert_that(_TerrainCostTable.get_cost_with_effects("water", true)).is_equal(1.0)
