...
func before_test() -> void:
	registry = SpellbookRegistry.new()
...
func test_parse_target_enemy_unit() -> void:
	assert_that(SpellEnums.parse_target("ENEMY_UNIT")).is_equal(SpellEnums.TargetType.ENEMY_UNIT)

func test_parse_target_all_ally() -> void:
	assert_that(SpellEnums.parse_target("ALL_ALLY_UNITS")).is_equal(SpellEnums.TargetType.ALL_ALLY_UNITS)

func test_parse_target_unknown_defaults_none() -> void:
	assert_that(SpellEnums.parse_target("INVALID")).is_equal(SpellEnums.TargetType.NONE)

func test_parse_effect_destroy() -> void:
	assert_that(SpellEnums.parse_effect("DESTROY")).is_equal(SpellEnums.EffectType.DESTROY)

func test_parse_status_frozen() -> void:
	assert_that(SpellEnums.parse_status("FROZEN")).is_equal(SpellEnums.StatusType.FROZEN)

func test_parse_speed_fast() -> void:
	assert_that(SpellEnums.parse_speed("fast")).is_equal(SpellEnums.SpellSpeed.FAST)

func test_parse_speed_slow() -> void:
	assert_that(SpellEnums.parse_speed("slow")).is_equal(SpellEnums.SpellSpeed.SLOW)

func test_parse_speed_burst() -> void:
	assert_that(SpellEnums.parse_speed("burst")).is_equal(SpellEnums.SpellSpeed.BURST)

func test_parse_speed_unknown_defaults_fast() -> void:
	assert_that(SpellEnums.parse_speed("invalid")).is_equal(SpellEnums.SpellSpeed.FAST)

func test_parse_color_fire() -> void:
	assert_that(SpellEnums.parse_color("FIRE")).is_equal(SpellEnums.SpellColor.FIRE)
...
func test_def_from_dict_ice_bolt() -> void:
	...
	var spell: Variant = SpellbookDef.from_dict(d)
...
func test_def_from_dict_empty() -> void:
	var spell: Variant = SpellbookDef.from_dict({})
...
func test_heal_clear_heals_and_clears_debuffs() -> void:
	...
	var result: Dictionary = TemplateEngine.execute(
...
func test_resolve_null_spell() -> void:
	var result: Dictionary = SpellResolver.resolve(null, null, null)
	assert_that(result["result"]).is_equal("invalid_spell")

func test_resolve_no_template() -> void:
	var spell: Variant = SpellbookDef.from_dict({"id": "no_template", "cost": 0})
	var result: Dictionary = SpellResolver.resolve(spell, null, null)
...
func test_resolve_insufficient_power() -> void:
	...
	var result: Dictionary = SpellResolver.resolve(spell, null, player)
...
func test_resolve_condition_not_met() -> void:
	...
	var result: Dictionary = SpellResolver.resolve(spell, null, player, null)
