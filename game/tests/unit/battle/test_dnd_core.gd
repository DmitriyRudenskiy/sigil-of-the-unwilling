...
func test_modifier_table_boundaries() -> void:
	assert_that(DNDAbilityScores.get_ability_modifier(5)).is_equal_to(-3)
	assert_that(DNDAbilityScores.get_ability_modifier(8)).is_equal_to(-1)
	assert_that(DNDAbilityScores.get_ability_modifier(10)).is_equal_to(0)
	assert_that(DNDAbilityScores.get_ability_modifier(14)).is_equal_to(2)
	assert_that(DNDAbilityScores.get_ability_modifier(18)).is_equal_to(4)
	assert_that(DNDAbilityScores.get_ability_modifier(20)).is_equal_to(5)
...
func test_modifier_all_scores() -> void:
	...
	for score in expected:
		assert_that(DNDAbilityScores.get_ability_modifier(score)).is_equal_to(expected[score])

func test_six_scores_and_get_set() -> void:
	var a = DNDAbilityScores.new(15, 14, 13, 12, 10, 8)
	assert_that(a.get_score(DNDAbilityScores.Ability.STR)).is_equal_to(15)
	assert_that(a.get_score(DNDAbilityScores.Ability.CHA)).is_equal_to(8)
	a.set_score(DNDAbilityScores.Ability.STR, 20)
	assert_that(a.get_str_mod()).is_equal_to(5)
	var mods = a.get_all_modifiers()
	assert_that(mods.size()).is_equal_to(6)

func test_scores_clamped() -> void:
	var a = DNDAbilityScores.new(99, 0, 10, 10, 10, 10)
	assert_that(a.get_score(DNDAbilityScores.Ability.STR)).is_equal_to(30)
	assert_that(a.get_score(DNDAbilityScores.Ability.DEX)).is_equal_to(1)

# ---------- TASK_02: Proficiency ----------

func test_proficiency_scaling_by_level() -> void:
	var p = DNDProficiencySystem.new(1)
	assert_that(p.get_proficiency_bonus()).is_equal_to(2)
	p.set_level(5)
	assert_that(p.get_proficiency_bonus()).is_equal_to(3)
	p.set_level(9)
	assert_that(p.get_proficiency_bonus()).is_equal_to(4)
	p.set_level(13)
	assert_that(p.get_proficiency_bonus()).is_equal_to(5)
	p.set_level(17)
	assert_that(p.get_proficiency_bonus()).is_equal_to(6)
	p.set_level(25)  # clamp
	assert_that(p.level).is_equal_to(20)
	assert_that(p.get_proficiency_bonus()).is_equal_to(6)

func test_proficiency_bonus_only_when_proficient() -> void:
	var p = DNDProficiencySystem.new(4)
	p.add_skill_proficiency("Stealth")
	assert_that(p.get_skill_bonus("Stealth")).is_equal_to(2)
	assert_that(p.get_skill_bonus("Athletics")).is_equal_to(0)
	p.add_expertise("Stealth")
	assert_that(p.get_skill_bonus("Stealth")).is_equal_to(4)
...
func test_basic_roll_formula() -> void:
	...
	assert_that(total).is_equal_to(r.d20_result + 3 + 2 + 1)
...
func test_advantage_takes_higher() -> void:
	for i in range(50):
		...
		assert_that(r.d20_result).is_greater_or_equal_to(r.second_d20)

func test_disadvantage_takes_lower() -> void:
	for i in range(50):
		var r = DNDAttackRoll.new()
		r.roll(rng, 0, 0, 0, false, true)
		assert_that(r.d20_result).is_less_or_equal_to(r.second_d20)

func test_critical_hit_and_miss() -> void:
	var r20 = DNDAttackRoll.new()
	r20.d20_result = 20
	assert_that(r20.is_crit_hit()).is_true()
	var r1 = DNDAttackRoll.new()
	r1.d20_result = 1
...
func test_advantage_disadvantage_cancel() -> void:
	...
	assert_that(r.second_d20).is_equal_to(0)
...
func test_roll_dice_ranges() -> void:
	...
	assert_that(DNDDamageCalculator.roll_dice("0d0", rng)).is_equal_to(0)
...
func test_calculate_damage_normal_and_crit() -> void:
	...
	assert_that(res["total"]).is_equal_to(res["base_damage"] + 2)
	assert_that(res["damage_type"]).is_equal_to(DNDDamageCalculator.DamageType.SLASHING)
...
func test_resistance_vulnerability_immunity() -> void:
	...
	assert_that(DNDDamageCalculator.apply_resistance(10, fire, [fire])).is_equal_to(5)
	assert_that(DNDDamageCalculator.apply_resistance(10, fire, [], [fire])).is_equal_to(20)
	assert_that(DNDDamageCalculator.apply_resistance(10, fire, [], [], [fire])).is_equal_to(0)
	# resistance rounds down, min 1
	assert_that(DNDDamageCalculator.apply_resistance(1, fire, [fire])).is_equal_to(1)

func test_all_13_damage_types() -> void:
	assert_that(DNDDamageCalculator.DAMAGE_TYPE_NAMES.size()).is_equal_to(13)
	for i in 13:
...
func test_initiative_formula_and_order() -> void:
	...
	assert_that(order[0]["id"]).is_equal_to("b")
	assert_that(order[1]["id"]).is_equal_to("a")
	assert_that(order[2]["id"]).is_equal_to("c")
...
func test_initiative_tie_broken_by_dex() -> void:
	...
	var order = t.get_turn_order()
	assert_that(order[0]["id"]).is_equal_to("high_dex")

func test_next_turn_cycles_and_rounds() -> void:
	var t = DNDInitiativeTracker.new()
	t.add_combatant("a", "A", 10, 2)
	t.add_combatant("b", "B", 12, 1)
	assert_that(t.next_turn().id).is_equal_to("b")
	assert_that(t.next_turn().id).is_equal_to("a")
	assert_that(t.next_turn().id).is_equal_to("b")
	assert_that(t.current_round).is_equal_to(2)
...
func test_add_remove_combatant_mid_combat() -> void:
	...
	assert_that(t.get_initiative_count()).is_equal_to(1)
	assert_that(t.remove_combatant("nope")).is_false()
	t.add_combatant("c", "C", 15, 0)
	assert_that(t.get_turn_order()[0]["id"]).is_equal_to("c")
...
func test_roll_initiative_uses_dex_and_bonus() -> void:
	...
	assert_that(t.get_turn_order()[0]["id"]).is_equal_to("x")
