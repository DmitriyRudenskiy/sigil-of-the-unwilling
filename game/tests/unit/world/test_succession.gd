...
func _succession_hero(path := &"archivist") -> HeroController:
	...
	h.magic.mana_max = 20
	var ring := Artifact.new(&"ring_of_might", &"Ring of Might", Artifact.Slot.RING_L,
		Artifact.Rarity.MINOR, {}, &"mana_regen", false, 100, "")
	h.inventory.backpack.append(ring)
	var blade := Artifact.new(&"blade_of_light", &"Blade of Light", Artifact.Slot.WEAPON,
		Artifact.Rarity.MAJOR, {}, &"", false, 250, "")
	h.inventory.equipped[Artifact.Slot.WEAPON] = blade
...
func test_build_copies_skills() -> void:
	...
	var succ := SuccessionController.new().build_successor(h)
...
func test_build_copies_path_magic_inventory() -> void:
	...
	assert_bool(succ.inventory.equipped[Artifact.Slot.WEAPON] != h.inventory.equipped[Artifact.Slot.WEAPON]).is_true()
...
func test_transfer_preserves_cities_identical() -> void:
	...
	SuccessionController.new().transfer_legend(h, succ, cities, mgr)
...
func test_transfer_reregisters_fresh_manager() -> void:
	...
	var dest = auto_free( CityManager.new())
	SuccessionController.new().transfer_legend(h, succ, cities, dest)
...
func test_resurrect_requires_temple_and_resources() -> void:
	...
	assert_bool(SuccessionController.new().resurrect_hero(no_temple)).is_false()

	var temple := _make_city(2, &"TempleTown", true)
	assert_bool(SuccessionController.new().resurrect_hero(temple)).is_true()
	...
	assert_bool(SuccessionController.new().resurrect_hero(poor)).is_false()

func test_can_resurrect_requires_temple() -> void:
	var city := _make_city(1, &"Village", false)
	city.buildings = []
	assert_bool(SuccessionController.new().can_resurrect(city)).is_false()
...
func test_can_resurrect_ok() -> void:
	...
	assert_bool(SuccessionController.new().can_resurrect(city)).is_true()

func test_resurrect_null_city() -> void:
	assert_bool(SuccessionController.new().resurrect_hero(null)).is_false()

func test_legend_init() -> void:
	var l := LegendTracker.new()
