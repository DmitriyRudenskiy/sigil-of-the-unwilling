...
func test_keys_are_fid_si() -> void:
	for k in CityYieldTable.KEYS:
		assert_bool(k in FIDSI).is_true()
	assert_that(CityYieldTable.KEYS.size()).is_equal(5)

func test_water_is_zero() -> void:
	var y := CityYieldTable.yield_for_terrain(HexUtils.Terrain.WATER)
	for k in CityYieldTable.KEYS:
		assert_float(y[k]).is_equal_approx(0.0, 0.0001)

func test_unknown_terrain_is_zero() -> void:
	var y := CityYieldTable.yield_for_terrain(99)
	for k in CityYieldTable.KEYS:
		assert_float(y[k]).is_equal_approx(0.0, 0.0001)

func test_grass_pinned() -> void:
	var y := CityYieldTable.yield_for_terrain(HexUtils.Terrain.GRASS)
...
func test_mountain_pinned() -> void:
	var y := CityYieldTable.yield_for_terrain(HexUtils.Terrain.MOUNTAIN)
...
func test_all_known_terrains_have_positive_yield() -> void:
	for t in [HexUtils.Terrain.SWAMP, HexUtils.Terrain.SAND, HexUtils.Terrain.GRASS,
		...
		var y := CityYieldTable.yield_for_terrain(t)
		var total := 0.0
		for k in CityYieldTable.KEYS:
			total += float(y[k])
