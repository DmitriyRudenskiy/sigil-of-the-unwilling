...
func test_band_boundaries() -> void:
	var cases: Array = [
		[90, ReputationSystem.Band.GOLDEN_AGE],
		[80, ReputationSystem.Band.GOLDEN_AGE],
		[79, ReputationSystem.Band.PROSPERITY],
		[30, ReputationSystem.Band.PROSPERITY],
		[29, ReputationSystem.Band.NORMAL],
		[0, ReputationSystem.Band.NORMAL],
		[-1, ReputationSystem.Band.DISCONTENT],
		[-30, ReputationSystem.Band.DISCONTENT],
		[-31, ReputationSystem.Band.CRISIS],
		[-70, ReputationSystem.Band.CRISIS],
		[-71, ReputationSystem.Band.REBELLION],
		[-100, ReputationSystem.Band.REBELLION],
	]
	for c in cases:
		var got: int = ReputationSystem.band(int(c[0]))
		assert_that(got).is_equal(int(c[1]))
	assert_that(ReputationSystem.band_name(90)).is_equal("Золотой век")
	assert_that(ReputationSystem.band_name(-80)).is_equal("Бунт")
...
func test_turn_factor_food() -> void:
	...
	var f: int = ReputationSystem.turn_factor(city)
...
func test_city_band_roundtrip() -> void:
	...
	assert_that(city2.reputation_band()).is_equal(ReputationSystem.Band.PROSPERITY)
...
func test_migration_in() -> void:
	...
	city.reputation = 50
	var total_before: int = city.pop_total()
	var res: Dictionary = ReputationSystem.process_migration(city)
...
func test_migration_out_order() -> void:
	...
	for i in 2:
		ReputationSystem.process_migration(city)
