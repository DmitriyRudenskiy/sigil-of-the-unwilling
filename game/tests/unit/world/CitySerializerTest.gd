...
# TASK_09: CitySerializer — state <-> Dictionary roundtrip.
...
func test_roundtrip_basic_fields() -> void:
	...
	# uid не восстанавливается из data — его назначает персистентный слой
...
func test_json_safe() -> void:
	# 4.3: сериализация JSON-совместима (без бинарных/циклических значений)
...
func test_empty_deserialize() -> void:
	# 4.3: пустой data не ломает город
	var c2 := City.new()
...
func test_uid_continuity() -> void:
	# 4.3: новые uid после deserialize больше максимального из сохранения
