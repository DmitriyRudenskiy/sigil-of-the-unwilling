...
func test_request_switch_to_worker() -> void:
	...
	assert_that(unit.pending_state).is_equal(_PopUnit.State.WORKER)
...
func test_request_switch_to_militia() -> void:
	...
	assert_bool(result).is_true()
	assert_that(unit.pending_state).is_equal(_PopUnit.State.MILITIA)
...
func test_request_switch_to_follower() -> void:
	...
	assert_bool(result).is_true()
	assert_that(unit.pending_state).is_equal(_PopUnit.State.FOLLOWER)
...
func test_apply_pending_to_worker() -> void:
	...
	assert_that(unit.state).is_equal(_PopUnit.State.WORKER)
...
func test_apply_pending_to_militia() -> void:
	...
	unit.apply_pending()
	assert_that(unit.state).is_equal(_PopUnit.State.MILITIA)
