...
func before_test() -> void:
	# TASK_04: сессия начинается с чистого кэша — UID старых городов
	# не должны пересекаться с новыми.
	ArenaClusterSystem.reset()
	city = CityArenaModel.make_city()


func _place_farm_cluster() -> Array:
	# 4 соседние клетки вокруг центра арены — связный кластер из 4 ферм
...
func test_new_city_after_reset_no_contamination() -> void:
	...
	# а у первой сессии кластер пересчитывается корректно
	var cl: Array = ArenaClusterSystem.clusters(city)
	assert_int(cl.size()).is_equal(1)
