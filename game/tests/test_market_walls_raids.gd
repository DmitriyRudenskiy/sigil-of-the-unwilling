...
const _Raid := preload("res://scripts/city/RaidSystem.gd")
...
func test_raid_chance_bounds() -> void:
	var c: Variant = _city()
	c.reputation = 100
	assert_bool(absf(_Raid.chance(c) - GameNumbers.RAID_CHANCE_MIN) < 1e-9).is_true()
	c.reputation = -100
	assert_bool(absf(_Raid.chance(c) - GameNumbers.RAID_CHANCE_MAX) < 1e-9).is_true()
	c.reputation = 0
