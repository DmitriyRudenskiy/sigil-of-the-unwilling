...
func _ready() -> void:
    _map_gen = MapGenerator.new(); _map_gen.name = "MapGenerator"
    _map_gen.seed_value = randi() % 999999; add_child(_map_gen)
    _hero = HeroController.new(); _hero.name = "Hero"; add_child(_hero); _hero.setup(_map_gen)
    _hero.hero_moved.connect(_on_hero_moved)
    _hero.hero_entered_village.connect(_on_village)
    _ui = AdventureUI.new(); add_child(_ui); _ui.setup(_hero)
    _ui.end_turn_pressed.connect(_on_end_turn)
    _camera = Camera2D.new(); _camera.position_smoothing_enabled = true
    add_child(_camera); _camera.position = _hero.position
    _spawn_villages()
...
func _process(delta: float) -> void:
    ...
    if mv != Vector2.ZERO: _camera.position += mv.normalized() * CAM_SPEED * delta / _zoom
...
func _spawn_villages() -> void:
    for cell in _map_gen.village_cells:
        var v := Node2D.new(); v.set_meta("cell", cell)
        var sp := Sprite2D.new()
        var img := Image.create(48,48,false,Image.FORMAT_RGBA8)
        for y in 48:
            for x in 48:
                if y<20 and abs(x-24)<(24-y)*0.8: img.set_pixel(x,y,Color(0.7,0.2,0.1))
                elif y>=20 and y<42 and x>=10 and x<=38: img.set_pixel(x,y,Color(0.6,0.5,0.3))
        sp.texture = ImageTexture.create_from_image(img); sp.z_index = 5; v.add_child(sp)
        v.position = _map_gen._tile_map.map_to_local(cell); add_child(v)
