...
static func get_all_neighbors(cell: Vector2i) -> Array[Vector2i]:
    ...
    for i in 6: r.append(get_neighbor(cell, i))
    return r
...
static func hex_distance(a: Vector2i, b: Vector2i) -> int:
    var ac := offset_to_cube(a); var bc := offset_to_cube(b)
...
static func bfs_path(start: Vector2i, goal: Vector2i, blocked: Dictionary, w: int, h: int) -> Array[Vector2i]:
    ...
    while queue.size() > 0:
        ...
        for bit in 6:
            ...
            if nxt.x < 0 or nxt.x >= w or nxt.y < 0 or nxt.y >= h: continue
            if blocked.has(nxt) or from.has(nxt): continue
            from[nxt] = cur
            queue.append(nxt)
    if not from.has(goal): return []
    var path: Array[Vector2i] = []
    var c := goal
    while c != start: path.append(c); c = from[c]
...
static func bfs_reachable(start: Vector2i, steps: int, blocked: Dictionary, w: int, h: int) -> Dictionary:
    ...
    while queue.size() > 0:
        var cur := queue.pop_front()
        if result[cur] >= steps: continue
        for bit in 6:
            var nxt := get_neighbor(cur, bit)
            if nxt.x < 0 or nxt.x >= w or nxt.y < 0 or nxt.y >= h: continue
            if blocked.has(nxt) or result.has(nxt): continue
