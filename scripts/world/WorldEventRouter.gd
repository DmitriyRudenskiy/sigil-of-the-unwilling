...
func _connect_ui_signals() -> void:
	if ui_manager:
		ui_manager.ui.end_turn_pressed.connect(_on_end_turn)
		ui_manager.ui.date_changed.connect(_on_date_changed)
		ui_manager.ui.minimap_cell_activated.connect(func(cell: Vector2i):
			if map_gen and map_gen.has_valid_tilemap():
				camera.center_on(map_gen.map_to_local(cell)))
		ui_manager.ui.camera_jump_requested_dir.connect(_on_camera_jump)
		ui_manager.ui.hex_borders_toggled.connect(func(on: bool):
			if ui_manager: ui_manager.set_hex_borders(on))
		ui_manager.ui.settings_applied.connect(_on_settings_applied)
		ui_manager.marker_layer.marker_hovered.connect(_on_marker_hovered)
		ui_manager.marker_layer.marker_clicked.connect(_on_marker_clicked)
