...
func start_battle(attacker_army: Array[UnitStack], defender_army: Array[UnitStack]) -> void:
	battle_started.emit()
	...
	battle.call_deferred("start_battle", attacker_army, defender_army)


func _on_battle_finished(winner: String, surviving_atk: Array, surviving_def: Array, battle: Node) -> void:
	battle.queue_free()
