...
# Полный CI: компиляция + проверки сцен/данных/тайлкетов + юнит-тесты + console clean (5 сценариев)
bash tools/shell/run_all_ci_checks.sh
# --fast  : только компиляция + проверки (без тестов и console clean)
# --tests : только тесты
...
2. Полный прогон (`run_all_ci_checks.sh`, до ~8 мин: ~5 мин проверки/тесты + ~2–3 мин
   console clean; с первого раза дольше из-за автозагрузки реестра) — все проверки +
   юнит-тесты (`4748 passed, 0 failed` из 76 файлов) + 5 живых сценариев.
