...
`assets/data/spells.json` — единый источник данных для всех заклинаний в игре. Файл содержит 508 заклинаний, распределённых по 19 шаблонам и 6 цветам/фракциям.

```
assets/data/spells.json         ← JSON-массив всех заклинаний (508 записей)
...
Базовые 16 шаблонов + 3 шаблона, вынесенные из battle-only (`BattleSpellBridge`) в
template-движок (глубина заклинаний):
...
| `HEAL_CLEAR` | 1 | `amount`, `target` | Лечение + снятие дебафов |
| `REVIVE` | 1 | `target`, `amount` | Возрождение юнита |
| `PORTAL` | 1 | `target`, `to` | Перемещение юнита (портал) |
...
Ребаланс скорости (глубина: ни одно значение не превышает 70%):
...
| `W920` | Общее количество ≠ 508 (порог = baseline) |
...
| Код | Кол-во | Описание |
|-----|--------|----------|
| `I922` | 1 | Дисбаланс цветов: max=125 (`shadow`), min=1 (`colorless`), ratio 125.0x |

Предупреждения, которые были в предыдущих версиях — **устранены** циклом глубина заклинаний:
`W901` (дубликат имени «Fireball») — после дифференциации описаний; `W910` (условное
`HARD_REMOVAL`) — 13 шаблонов без `condition` помечены `unconditional: true`; `W920`/`W921`
— порог/расхождение по шаблонам синхронизированы с baseline (508); `W924` (87% `fast`)
— после rebalance скорости (`fast` 69.1%).

Остаётся только информационное сообщение `I922` (дисбаланс цветов) — оно не является ошибкой
и не блокирует `--strict`.
