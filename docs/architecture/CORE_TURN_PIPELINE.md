...
Сквозной слой `game/scripts/core/`, общий для мира, боя, экономики, демографии и города.
...
- **entities / city / economy / demographics НЕ зависят от `scripts/core/` напрямую** в
...
- **Ядро (`scripts/core/`) не зависит от визуала** (`scripts/ui/`, `assets/`) и от тяжёлой логики
  подсистем.
